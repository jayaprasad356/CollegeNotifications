...
if (isset($_GET['table']) && $_GET['table'] == 'orders') {
    ...
    $sql = "select o.*,u.name FROM orders o LEFT JOIN users u ON u.id=o.user_id  LEFT JOIN pincodes p ON p.id=o.pincode_id " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
...
if (isset($_GET['table']) && $_GET['table'] == 'seller') {
    ...
    foreach ($res as $row) {
        ...
        $tempRow['bank_name'] = $row['bank_name'];
        $tempRow['account_name'] = $row['account_name'];
        $tempRow['require_products_approval'] = ($row['require_products_approval'] == 1) ? "<span class='label label-info'>No</span>" : "<span class='label label-primary'>Yes</span>";
